...
class PngImgSpider(scrapy.Spider):
    ...
    def start_requests(self):
        urls = [line.strip() for line in open('/data/1019_crawl_img/crawl_images/pngimg/child_links.txt', 'r')]
        for url in urls:
            cat = url.strip().split('/')[-3]
            child_cat = url.strip().split('/')[-2]
            if os.path.exists(os.path.join('/data/1019_crawl_img/crawl_images/pngimg', cat, child_cat, 'links.txt')):
                pass
...
#                 print('Crawl {}'.format(url))
    ...
    def parse(self, response):
        ...
        output_dir = '/data/1019_crawl_img/crawl_images/pngimg'
