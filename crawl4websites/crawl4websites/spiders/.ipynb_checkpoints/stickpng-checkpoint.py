...
class StickPngSpider(scrapy.Spider):
    ...
    def start_requests(self):
        urls = [line.strip() for line in open('/data/1019_crawl_img/crawl_images/stickpng/link_child_cat.txt', 'r')]
#         urls = ['https://www.stickpng.com/cat/animals/cats']
        for url in urls:
            cat = url.strip().split('/')[-3]
            child_cat = url.strip().split('/')[-2]
            if os.path.exists(os.path.join('/data/1019_crawl_img/crawl_images/stickpng', cat, child_cat, 'links.txt')):
